...
} from "../dtos/outputs/movie_with_genres.output";
...
async function findManyLikedMoviesByUserId(
  ...
  // 1.
  if (after || cursor) {
    likes = await tx.likeMovie.findMany({
      where: {
        userId,
      },
      orderBy: {
...
async function restore({ movieLikeId, tx }: PickIdsWithTx<"movieLike">) {
...
async function softDeleteById({ movieLikeId, tx }: PickIdsWithTx<"movieLike">) {
  return tx.likeMovie.update({
    where: {
      id: movieLikeId,
    },
