...
async function favoriteMovies(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const { latestFavoriteId } =
      await MoviesMetadataRepository.findByUserIdOrCreateAndReturn({
        userId,
        tx,
      });
...
async function toggleFavorite(
  ...
  await prisma.$transaction(async (tx) => {
    ...
    let favorite = await MoviesFavoriteRepository.findByUserIdAndMovieId({
      userId,
      movieId,
      tx,
    });
    ...
    if (favorite !== null && !isDeleted(favorite)) {
      await MoviesFavoriteRepository.softDeleteById({
    ...
    if (favorite === null) {
      ...
      await MoviesFavoriteRepository.restore({
        favoriteMovieId: favorite.id,
        tx,
      });
    }
...
async function deleteFavorite({
  ...
  await prisma.$transaction(async (tx) => {
    const favorite = await MoviesFavoriteRepository.findByUserIdAndMovieId({
      userId,
      movieId,
      tx,
    });
