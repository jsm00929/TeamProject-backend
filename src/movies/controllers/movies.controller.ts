...
async function histories(req: AuthRequestWith<PaginationQuery>) {
  ...
  const movies = await moviesHistoryService.histories({ userId }, q);
...
async function likes(req: AuthRequestWith<PaginationQuery>) {
  ...
  const movies = await moviesLikeService.likeMovies({ userId }, q);
...
async function favorites(req: AuthRequestWith<PaginationQuery>) {
  ...
  const movies = await moviesFavoriteService.favoriteMovies({ userId }, q);
...
async function toggleMovieLike(
  req: AuthRequestWith<ToggleMovieLikeBody, MovieIdParams>
) {
...
async function toggleFavorite(
  req: AuthRequestWith<ToggleFavoriteMovieBody, MovieIdParams>
) {
  const userId = req.userId;
  const { movieId } = req.unwrapParams();
  const body = req.unwrap();
